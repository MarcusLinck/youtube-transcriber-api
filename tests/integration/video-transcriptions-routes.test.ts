...
const DB_NAME = 'videos_test_transcriptions'
...
const app = buildApp({
  generateTranscription: async () => 'Conteúdo transcrito de teste.',
})
...
test('POST /api/v1/videos/:id/transcriptions retorna 201 e cria a transcrição', async () => {
  ...
  assert.equal(body.transcription.content, 'Conteúdo transcrito de teste.')
...
test('POST /api/v1/videos/:id/transcriptions retorna 409 quando o vídeo já possui transcrição', async () => {
...
test('GET /api/v1/videos/:id/transcriptions retorna 404 para vídeo inexistente', async () => {
  const response = await app.inject({ ...authHeaders,
    method: 'GET',
    url: '/api/v1/videos/id-inexistente/transcriptions',
  })
...
test('DELETE /api/v1/videos/:id/transcriptions retorna 404 quando não há transcrição', async () => {
  const response = await app.inject({ ...authHeaders,
    method: 'DELETE',
    url: `/api/v1/videos/${videoId}/transcriptions`,
  })
...
test('é possível gerar uma nova transcrição após excluir a anterior', async () => {
  await app.inject({ ...authHeaders,
    method: 'POST',
    url: `/api/v1/videos/${videoId}/transcriptions`,
  })
  ...
  assert.equal(response.statusCode, 201)
  assert.equal(response.json().transcription.content, 'Conteúdo transcrito de teste.')
})