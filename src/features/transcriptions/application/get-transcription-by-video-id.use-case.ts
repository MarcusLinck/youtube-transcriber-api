...
import type {
  GetTranscriptionByVideoIdDTO,
  ReturnFoundTranscriptionDTO,
...
export class GetTranscriptionByVideoIdUseCase
  implements UseCase<GetTranscriptionByVideoIdDTO, ReturnFoundTranscriptionDTO> {
  constructor(
    private readonly videosRepository: IVideosRepository,
    private readonly transcriptionsRepository: ITranscriptionsRepository,
  ) {}

  async execute(input: GetTranscriptionByVideoIdDTO): Promise<ReturnFoundTranscriptionDTO> {
